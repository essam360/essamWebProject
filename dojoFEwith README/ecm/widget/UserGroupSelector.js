...
define([
	 ...
	 */
	var UserGroupSelector = declare("ecm.widget.UserGroupSelector", [
		ContentPane,
		_TemplatedMixin,
		...
		/** @lends ecm.widget.UserGroupSelector.prototype */
		...
		_setUserSearchLabel: function() {
			if (this._scope.isLoaded()) {
				// case 1: scope is users and share plugin is enabled
				if (this._scope.get("value") == "users" && this._isSharePluginEnabled()) {
					this._searchTypeLabel.innerHTML = this._messages.name_contains_label;
					this._criteriaTextBox.set("value", "");
					this._criteriaTextBox.set("placeholder", this._messages.name_contains_label);

					// case 2:  scope is not users OR share plugin not enabled
				} else {
				...
				// case 3: just loaded selector so default to users. share plugin enabled. 
				if (this._isSharePluginEnabled()) {
					this._searchTypeLabel.innerHTML = this._messages.name_contains_label;
					this._criteriaTextBox.set("value", "");
					this._criteriaTextBox.set("placeholder", this._messages.name_contains_label);

					// case 4: defaulting to users, share plugin not enabled
				} else {
					this._criteriaTextBox.set("placeholder", this._messages.name_starts_with_label);
					this._criteriaTextBox.set("value", "");
					this._searchTypeLabel.innerHTML = this._messages.name_starts_with_label;
		...
		_onSearch: function(event) {
			...
			if (query.length > 0) {
				...
				// custom search is used for P8 user search only. not for CM8 and not for groups.
				// P8 group search uses only the server default, prefix.
				if (this.queryMode == this._QUERY_MODE.USERS || this._scope.get("value") == this._SEARCH_FOR_MODE.USERS) {
					if (thisSearchType == 1 && this.repository._isP8()) { // for P8 custom search, add *s
						var wordArray = query.split(" ");
						query = "*";
						for (i = 0; i < wordArray.length; i++) {
							var s = wordArray[i].trim(); // strip out whitespace
							if (s.length > 0) { // exclude any empty string array elements
								query = query.concat(wordArray[i], "*");
							}
						}
					}

					this.repository.findUsers(lang.hitch(this, "_onComplete"), directoryName, query, thisSearchType, null, (this._MAX_ROWS + 1).toString(), null, this.queryAcrossDomains, this.excludeCMAdminAccounts);
				} else
					this.repository.findGroups(lang.hitch(this, "_onComplete"), directoryName, query, null, null, (this._MAX_ROWS + 1).toString(), null, this.queryAcrossDomains);
			}
		},
		...
		getSelectedItems: function() {
			...
			if (this.showPseudoUsers && this._meRadioButton.get("checked")) {
				selectedItems.push(lang.clone(UserGroupSelector.PSEUDO_USER_ME));
		...
		_newItem: function(items, item) {
			if (items.length == 0 || array.every(items, function(currentItem) {
				return currentItem.shortName != item.shortName;
			})) {
				items.push(item);
				return true;
			}
		...
		_updatePseudoUserOptions: function() {
			...
			if (!items.length || items[0].id == UserGroupSelector.PSEUDO_USER_ME.id) {
				if (!items.length)
